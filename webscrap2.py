...
def extract_system_info_from_html(file_path):
    ...
    processor = soup.find_all('div', "reportSection rsLeft")[1].find('td').get_text(separator='\n', strip=True).split('\n')[0]
    board = ''.join(soup.find_all('div', class_="reportSection rsRight")[1].find('td').get_text(separator='\n', strip=True).split('\n')[0].split(' ')[1:])
    hard_disk = soup.find_all('div', 'reportSection rsLeft')[2].find('td').get_text(separator='\n', strip=True).split('\n')[0].split(' ')[0] + 'GB'
...
def process_files(files, output_file, status_label):
...
def select_files_and_process():
    ...
    label = Label(root, text="Select HTML files:")
    label.pack(pady=10)

    # Function to handle button click
    def select_files():
        files_selected = filedialog.askopenfilenames(filetypes=[("HTML files", "*.html")])
        if files_selected:
            output_file = os.path.join(os.path.dirname(files_selected[0]), 'output.xlsx')
            status_label.config(text="Processing...")
            process_files(files_selected, output_file, status_label)
            subprocess.Popen(['start', '', output_file], shell=True)

    # Button to select files
    button = Button(root, text="Select Files", command=select_files)
    button.pack(pady=10)
